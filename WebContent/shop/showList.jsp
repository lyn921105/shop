...
<div id="shop" class="box2">
	...
	<c:if test="${book_kind!='all' }">
		<c:set var="diplay" value="[${book_kindName}] 분류의 목록" />
	</c:if>
		...
		<table class="vhcenter">
			<tr height="30">
				<td rowspan="4" width="100"><a
					href="/shoppingmall/bookContent.do?book_id=${book.getBook_id() }&book_kind=${book.getBook_kind()}">
						<img src="/shoppingmall/bookImages/${book.getBook_image() }"
						class="listimage">
				</a></td>
			...
			<tr height="30">
				<td width="350"><c:set var="price"
						...
						정가: <fmt:formatNumber
						value="${price}" type="number" pattern="#,##0" />원<br>
				<strong class="bred">판매가: <fmt:formatNumber
							value="${price*((100-rate)/100) }" type="number" pattern="#,##0" />원
				</strong></td>
			</tr>
		</table>
	</c:forEach>
