...
<script type="text/javascript" src="/shoppingmall/shop/bookcontent.js"></script>
		...
		<c:forEach var="qna" items="${qnaLists }">
			<ul>
				<li><c:set var="writer" value="${qna.getQna_writer() }" />
					${fn:substring(writer,0,4) }**** <small class="date">
						(${qna.getReg_date() })</small>
				<li>${qan.getQna_content() }
				<li><c:if test="${sessionScope.id==writer }">
						<button id="edit" name="${qna.getQan_id() },${book_kind}"
							onclick="edit(this)">수정</button>
						<button id="delete"
							name="${qna.getQan_id() },${book_id},${book_kind}"
							onclick="delete(this)">삭제</button>
					</c:if>
			</ul>
		</c:forEach>
	</c:if>
